...
extension ViewController {
    ...
    func downloadAndParse(subreddit:String) {
        ...
        if let validURL = URL(string: "https://www.reddit.com/r/\(subreddit)/.json") {
            ...
            let task = session.dataTask(with: validURL) { (data, response, error) in
                ...
                func CheckForHttpString(title:String, thumbnail:String, author:String, subReddit:String)
                {
                    ...
                    if thumbnail.contains("https")
                    {
                     //print and check what url is being passed threw
                        ...
                        self.redditData.append(Reddit(title: title, author: author, thumbnailString: thumbnail, subReddit: subReddit))
                    }
                }
                    ...
                    for firstLevel in json
                    {
                        ...
                        for object in children
                        {
                            ...
                            let author = dataTwo ["author"] as? String,
                                let subReddit = dataTwo ["subreddit"] as? String
                                else{continue}
                           
                            //pass requird data into nested function for check.
                            CheckForHttpString(title: title, thumbnail: thumbnail, author: author, subReddit:subReddit)
                        }
